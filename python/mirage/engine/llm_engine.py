...
class LLMEngine:
    ...
    def generate(self, token_ids, packed, params, timeout=120):
        import torch
        with self._lock:
            if self._closed:
            ...
            rid = self._next_rid
            self._next_rid += 1
            session = GenerationSession(self, rid, len(token_ids), params, timeout)
            self._sessions[rid] = session
            try:
                self.runtime.submit(rid, torch.tensor(token_ids, dtype=torch.int64),
                                    generation_config=packed)
            except Exception:
                del self._sessions[rid]
                raise
            return session

    def submit(self, prompt: str | None = None, use_template=True, timeout=120, poll_interval=1e-4,
        ...
        ids, packed = self.prepare(prompt=None if messages is not None else prompt,
                                   messages=messages, params=params, use_template=use_template)
        session = self.generate(ids, packed, params, timeout)
        if stream:
            def iterator():
                try:
