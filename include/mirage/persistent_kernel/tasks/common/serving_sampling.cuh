...
__device__ inline float cfg_float(int64_t value) {
...
__device__ inline uint64_t cutoff(float *scores, int vocab, uint64_t lower, float target, bool weighted, float max_score, float* shared) {
    ...
    for (int bit = 63; bit >= 0; bit--) {
        ...
        for (int v = threadIdx.x; v < vocab; v += blockDim.x) {
            ...
            if (key >= lower && (key & mask) == prefix && (key & test)) {
                mass += weighted ? expf(scores[v] - max_score) : 1.0f;
            }
        }
        mass = sum(mass, shared);
        mask |= test;
        if (mass > 0 && mass >= target) {
            prefix |= test;
        }
        else {
            target -= mass;
...
__device__ inline void sample(T const *logits, float *scratch, Token* output,
                                int padded_vocab, int64_t const* cfg, long long const* history,
                                int history_len, int prompt_len, int generation_position) {
        ...
        int *generated_counts = reinterpret_cast<int *>(scratch + 2 * padded_vocab);
        ...
        bool penalties = cfg_float(cfg[6]) != 0 || cfg_float(cfg[7]) != 0 || cfg_float(cfg[8]) != 1;
        if (penalties) {
            for (int i = threadIdx.x; i < history_len; i += blockDim.x) {
                ...
                if (token >= 0 && token < vocab) {
                    ...
                    if (i >= prompt_len) {
                        atomicAdd(generated_counts + token, 1);
                    }
                }
            }
        }
        ...
        float temperature = cfg_float(cfg[3]);
        uint64_t best = 0;
        for (int v = threadIdx.x; v < vocab; v += blockDim.x) {
            float score = static_cast<float>(logits[v]);
            for (int j = 0; j < cfg[10]; j++) {
                if (cfg[32 + 2 * j] == v) {
                    score += cfg_float(cfg[33 + 2 * j]);
                }
            }
            if (counts[v]) {
                score = score > 0 ? score / cfg_float(cfg[8]) : score * cfg_float(cfg[8]);
            }
            score -= cfg_float(cfg[6]) * generated_counts[v] + cfg_float(cfg[7]) * (generated_counts[v] > 0);
            if (temperature > 0) {
                score /= temperature;
            }
            if (isnan(score)) {
